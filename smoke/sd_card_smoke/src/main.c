...
TASK(smoke_sd_task, TASK_STACK_512) {
  ...
  while (true) {
    ...
    for (int j = 0; j < 64; j++) {
      LOG_DEBUG("%x %x %x %x %x %x %x %x\n", spi_rx_data[j+0],spi_rx_data[j+1],spi_rx_data[j+2],spi_rx_data[j+3],spi_rx_data[j+4], spi_rx_data[j+5],spi_rx_data[j+6],spi_rx_data[j+7]);
    }
    delay_ms(1000);
    ...
    for (int j = 0; j < 64; j++) {
      LOG_DEBUG("%x %x %x %x %x %x %x %x\n", spi_tx_data[j+0],spi_tx_data[j+1],spi_tx_data[j+2],spi_tx_data[j+3],spi_tx_data[j+4], spi_tx_data[j+5],spi_tx_data[j+6],spi_tx_data[j+7]);
    }
    delay_ms(1000);
  }
  free(spi_tx_data);
