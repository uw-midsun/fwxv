...
class DischargeTestGraph:
    ...
    def save_to_csv(self):
        with open(self.csv_filename, mode='w', newline='') as csv_file:
            ...
            for i in range(len(self.time_intervals)):
                csv_writer.writerow([self.time_intervals[i], self.voltage_readings[i], self.batt_ah[i]])   

    def update_plots(self):
        self.ax1.clear()
        self.ax1.plot(self.time_intervals, self.voltage_readings, label='Voltage (V)', color='green')
    ...
    def add_data(self, voltage, current_time):
        ...
        if len(self.time_intervals) > 1:
            ...
            self.batt_ah = np.append(self.batt_ah, self.batt_ah[-1] + delta_ah if len(self.batt_ah) > 0 else delta_ah)
        else:
