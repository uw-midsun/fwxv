...
def mock_voltage_discharge_curve(initial_voltage, final_voltage, discharge_duration, num_points, voltage_variation=0.05, time_variation=0.05):
    ...
    voltage_values = np.array([v + random.uniform(-voltage_variation, voltage_variation) for v in voltage_values])
    time_intervals = np.array([t + random.uniform(-time_variation, time_variation) for t in time_intervals])
...
def test_discharge_test_graph():
    ...
    time_intervals, voltage_values = mock_voltage_discharge_curve(initial_voltage, final_voltage, discharge_duration, num_points, voltage_variation, time_variation)
