...
void test_precharge() {
  ...
  TEST_ASSERT_EQUAL(NEUTRAL, g_tx_struct.drive_output_drive_state);
  TEST_ASSERT_EQUAL(NEUTRAL, drive_fsm->curr_state->id);
  TEST_ASSERT_EQUAL(STATUS_CODE_TIMEOUT, fsm_shared_mem_get_drive_error_code(&cc_storage));
  ...
  neutral_to_precharge();
  precharge_to_drive();
  TEST_ASSERT_EQUAL(STATUS_CODE_OK, fsm_shared_mem_get_drive_error_code(&cc_storage));
  notify(drive, NEUTRAL_BUTTON_EVENT);
  drive_to_neutral();
