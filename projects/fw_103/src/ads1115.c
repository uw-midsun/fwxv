...
StatusCode ads1115_init(ADS1115_Config *config, ADS1115_Address i2c_addr, GpioAddress *ready_pin) {
  ...
  cmd = 0x8483; //1000 0100 1000 0011
...
StatusCode ads1115_read_raw(ADS1115_Config *config, ADS1115_Channel channel, uint16_t *reading) {
  ...
  i2c_read_reg(config->i2c_port, config->i2c_addr, ADS1115_REG_CONVERSION, (uint16_t *) reading, 2);
  return STATUS_CODE_OK;
}
...
StatusCode ads1115_read_converted(ADS1115_Config *config, ADS1115_Channel channel, float *reading) {
  ...
  i2c_read_reg(config->i2c_port, config->i2c_addr, ADS1115_REG_CONVERSION, &raw_reading, 2);
  *reading = raw_reading * (4.096f/65535.0f)
  return STATUS_CODE_OK;
}
