...
static StatusCode prv_cell_sense_conversions() {
  ...
  if (ltc_afe_impl_read_cells(ltc_afe_storage)) {
    // If this has failed, try once more after a short delay
    LOG_DEBUG("READ_FAILED RETRYING.\n");
    delay_ms(5);
...
StatusCode cell_sense_run() {
  ...
  if (ltc_afe_storage->timer_start == 3) {
    ltc_afe_impl_write_config(ltc_afe_storage);
  }

  ltc_afe_storage->timer_start = (ltc_afe_storage->timer_start + 1) % 11;
