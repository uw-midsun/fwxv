...
StatusCode can_init(CanStorage *storage, const CanSettings *settings)
{
  ...
  status_ok_or_return(can_stack_init(&s_can_storage->rx_stack)) 
  // Initialize hardware settings
  status_ok_or_return(can_hw_init(&s_can_storage->rx_stack, settings));
...
StatusCode can_receive(const CanMessage *msg)
{
  ...
  StatusCode ret = can_stack_pop(&s_can_storage->rx_stack, msg);
