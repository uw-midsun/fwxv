...
static void assert_drive_command(float current_percent, float velocity_rpm) {
  ...
  can_queue_pop(&s_mcp2515_tx_queue, &can_message);

  TEST_ASSERT_EQUAL(DRIVER_CONTROL_BASE + 0x01, can_message.id.raw);
  TEST_ASSERT_EQUAL_MEMORY(&can_message.data_u32[0], &current_percent, sizeof(float));
  TEST_ASSERT_EQUAL_MEMORY(&can_message.data_u32[1], &velocity_rpm, sizeof(float));
}
