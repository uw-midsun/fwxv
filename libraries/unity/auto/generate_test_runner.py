...
def self.grab_config(config_file)
    ...
    if (not options): raise "No :unity or :cmock section found in #{config_file}"
    # return options

def run(input_file, output_file, options = None):
...
def find_tests(source):
    ...
    substring_subs = { '{': '@co@', '}': '@cc@', ';': '@ss@', '/': '@fs@' } # '\\"': '@quote@', "\\'": '@apos@', 
    ...
    source_scrubbed = source_scrubbed.replace('\\"', '@quote@')   # hide escaped quotes to allow capture of the full string/char
...
def create_main(output, filename, tests, used_mocks)
    ...
    output.puts("#{main_return} test_task(void)")
    output.puts('{')
    end
