...
StatusCode adt7476a_init(Adt7476aStorage *storage, Adt7476aSettings *settings) {
  ...
  storage->event = settings->event;  // called when tachometer goes out of range
  storage->task = settings->task;
  ...
  gpio_it_register_interrupt(&storage->smbalert_pin, &s_interrupt_settings, storage->event,
                             storage->task);
