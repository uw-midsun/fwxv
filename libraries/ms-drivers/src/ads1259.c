...
static void prv_read_conversion(Ads1259Storage *storage) {
...
StatusCode ads1259_get_conversion_data(Ads1259Storage *storage) {
  // Send command to begin conversion
  prv_send_command(storage, ADS1259_START_CONV);

  // Delay until conversion is complete
  delay_ms(s_conversion_time_ms_lookup[ADS1259_DATA_RATE_SPS]);

  // Read conversion
  prv_read_conversion(storage);
  return STATUS_CODE_OK;
}
