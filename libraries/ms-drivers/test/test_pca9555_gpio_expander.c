...
void test_gpio_set_state(void) {
  ...
  status = i2c_set_data(i2c_port, &data, 1);
  ...
  // check data written to reg. Reg content should have bit 4 turned on since mock pin is pin 4.
  status = i2c_get_data(i2c_port, &data, 1);
  TEST_ASSERT_EQUAL(STATUS_CODE_OK, status);
...
void test_gpio_get_state(void) {
  ...
  // reading the input reg content. Reg content should be high because bit 4 is high in mock state
