...
#define DECLARE_SOFT_TIMER(timer_name)                    \
  ...
  static TimerHandle_t _s_timer_##timer_name
...
#define soft_timer_start(duration_ms, callback, timer_name, soft_timer_id)                      \
  _s_timer_##timer_name = xTimerCreateStatic("", duration_ms, pdFALSE, soft_timer_id, callback, \
                                             &_s_timer_buffer_##timer_name);                    \
  xTimerStart(_s_timer_##timer_name, 0)

// Cancels the soft timer specified by name. Returns true if successful.
#define soft_timer_cancel(timer_name) xTimerDelete(_s_timer_##timer_name, 0)

// Checks if software timers are running. Returns true if any soft timers
// are in use.
#define soft_timer_inuse(timer_name) xTimerIsTimerActive(_s_timer_##timer_name) == pdTrues
...
#define soft_timer_remaining_time(timer_name) \
  xTimerGetExpiryTime(_s_timer_##timer_name) - xTaskGetTickCount()
