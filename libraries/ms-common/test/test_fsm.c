...
static FsmTransition s_test1_transitions[] = {
...
void prv_init_fsm(uint8_t init_state) {
  ...
  TEST_ASSERT_EQUAL(init_state, test1_fsm->curr_state->id);
...
void test_state_table_transitions_success(void) {
  ...
  TEST_ASSERT_EQUAL(TEST_STATE_0, test1_fsm->curr_state->id);
  // 0 -> 1
  TEST_ASSERT_OK(fsm_transition(test1_fsm, TEST_STATE_1));
  TEST_ASSERT_EQUAL(TEST_STATE_1, test1_fsm->curr_state->id);
  // 1 -> 0
  TEST_ASSERT_OK(fsm_transition(test1_fsm, TEST_STATE_0));
  TEST_ASSERT_EQUAL(TEST_STATE_0, test1_fsm->curr_state->id);
  // 0 -> 2
  TEST_ASSERT_OK(fsm_transition(test1_fsm, TEST_STATE_2));
  TEST_ASSERT_EQUAL(TEST_STATE_2, test1_fsm->curr_state->id);
