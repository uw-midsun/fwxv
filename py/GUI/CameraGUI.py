...
for y in range(3):
    for x in range(3):
        ...
        ).place(x=34 + (x * 137), y=40 + (y * 50))

tk.Label(Box, text="AFE1", font="Montserrat 15", bg="#000000", fg="white").place(
    x=75, y=6
)
tk.Label(Box, text="AFE2", font="Montserrat 15", bg="#000000", fg="white").place(
    x=209, y=6
)
tk.Label(Box, text="AFE3", font="Montserrat 15", bg="#000000", fg="white").place(
    x=342, y=6
)
...
tk.Label(Box, text="MAX\nTEMP", font="Montserrat 8", bg="#000000", fg="white").place(
    x=3, y=52
)
tk.Label(Box, text="MAX\nVOLT", font="Montserrat 8", bg="#000000", fg="white").place(
    x=3, y=106
)
...
for x in range(2):
    Box = tk.Canvas(
        ...
        highlightbackground="white",
        highlightthickness=BorderThickness,
    ).place(x=0 + (x * 223), y=190)
tk.Label(Box, text="CURRENT", font="Montserrat 10", bg="#000000", fg="white").place(
    x=10, y=210
)
tk.Label(Box, text="VOLT.", font="Montserrat 10", bg="#000000", fg="white").place(
    x=241, y=210
)
...
def redBox(message, num):
    ...
    tk.Label(RedBox, text=message, font="Montserrat 10", bg=red, fg="black").place(
        x=450, y=(List[redlistCount])
    )
    tk.Label(RedBox, text=num, font="Montserrat 10", bg=red, fg="black").place(
        x=743, y=(List[redlistCount])
    )
    redlistCount += 1
...
def orangeBox(message, num):
    ...
    ).place(x=448, y=List[listCount + 3])
    tk.Label(
        OrangeBox, text=message, font="Montserrat 10", bg=orange, fg="black"
    ).place(x=450, y=(List[listCount + 3]))
    tk.Label(OrangeBox, text=num, font="Montserrat 10", bg=orange, fg="black").place(
        x=743, y=(List[listCount + 3])
    )
    listCount += 1
...
def yellowBox(message, num):
    ...
    tk.Label(
        YellowBox, text=message, font="Montserrat 10", bg=yellow, fg="black"
    ).place(x=450, y=(List[listCount + 3]))
    tk.Label(YellowBox, text=num, font="Montserrat 10", bg=yellow, fg="black").place(
        x=743, y=(List[listCount + 3])
    )
    listCount += 1
...
DISPLAY_MSG_DICT = {
    ...
    db.get_message_by_name("battery_status").frame_id: [
        (BMS_Fault, "fault"),
        (nothing, "aux_batt_v"),
    ],
    # something for fault
    db.get_message_by_name("pd_status").frame_id: [
        (nothing, "power_state"),
        (display_pd_status, "fault_bitset"),
        (nothing, "bps_persist"),
    ],
    db.get_message_by_name("mc_status").frame_id: [
        (nothing, "limit_bitset_l"),
        (mc_error_bitset_l, "error_bitset_l"),
        (nothing, "limit_bitset_l"),
        (mc_error_bitset_r, "error_bitset_r"),
        (nothing, "board_fault_bitset"),
...
def handle_message(msg):
    ...
    try:
        ...
        if msg.arbitration_id in DISPLAY_MSG_DICT_CURRENT_SENSE:
            ...
            tk.Label(
                Box,
                text=((decoded_message["voltage"] / 1000), "A"),
                font="Montserrat 16",
                width=6,
                bg="#000000",
                fg="white",
