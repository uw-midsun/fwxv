...
candump_pattern = re.compile(r"(\w*)  (\w*)   \[([0-9])\]  ((?:\w\w ?){8})")
    ...
    with subprocess.Popen(['candump', 'vcan0'], stdout=subprocess.PIPE) as proc:
        ...
        while True:
            ...
            for i, base_addrs in enumerate([MOTOR_CONTROLLER_BASE_L, MOTOR_CONTROLLER_BASE_R]):
                ...
                send_message(base_addrs + 0x03, motor_velocity + i,
                             motor_velocity * 2 + i)  # vehicle vel, motor rpm
                ...
                send_message(base_addrs + 0x05, 0.2,
                             0.2)  # voltage vec Vd, Vq
                send_message(base_addrs + 0x06, 0.2,
                             0.2)  # current vec Id, Iq
                send_message(base_addrs + 0x07, 0.1, 0.1)  # backEmf d, q

            counter += 1
            if counter % 5:
                # every 1 second
                offset = counter_2 * 0.1
                counter_2 += 1
                if counter_2 % 10:
                    counter_2 = 0
                ...
                for i, base_addrs in enumerate([MOTOR_CONTROLLER_BASE_L, MOTOR_CONTROLLER_BASE_R]):
                    ...
                    send_message(base_addrs + 0x08, 15.0 +
                                 offset + i, 0)  # 15V rail voltage
